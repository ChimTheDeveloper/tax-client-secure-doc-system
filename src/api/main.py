...
async def upload_document(file: UploadFile = File(...)):
    try:
        ...
        # STEP 7: VALIDATE
        validated_data, _ = validate_w2_data(mapped_data)
        print("[VALIDATED DATA]", validated_data)

        # STEP 8: CONFIDENCE (NUMERIC)
        confidence_score = calculate_confidence(validated_data)
        print("[CONFIDENCE SCORE]", confidence_score)

        # HARD REJECTION BASED ON SCORE
        if confidence_score < 0.6:
            raise HTTPException(
                status_code=422,
                detail="Low confidence extraction - manual review required"
            )
